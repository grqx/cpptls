...
int main() {
  ...
  std::shared_ptr<void> openssl_raii{initialize_openssl(),
                                     [](void *) { EVP_cleanup(); }};
  ...
  sockaddr_in server_addr;
  {
    ...
    unique_ptr_with_deleter<void> addrInfoRAII{
        nullptr, [&](void *) { freeaddrinfo(res); }};
  ...
  {
    TLS_Session tlss(TLS_Version::TLS_1_2,
    ...
    auto printParsedRecLayer = [](auto &&parsedRecLayer) {
      ...
      for (auto &&cont : parsedRecLayer.parsedContent) {
        std::cout << "content entry " << i_++ << " of type "
                  ...
                  << "(rVer: " << static_cast<int>(cont.recordVersion)
                  << "): [packet of " << cont.realCont.size() << " bytes]\n";
      }
      std::cout << "}\n";
    };
    ...
    if (hasNetworkConn) {
      ...
      for (auto it = ret.parsedContent.begin(), e = ret.parsedContent.end();
           it != e; it++) {
    ...
    for (auto &&kexPacket : *kexPackets) {
      ...
      recLayerMsgs.splice(recLayerMsgs.end(), recLayerMsgList,
                          recLayerMsgList.begin(), recLayerMsgList.end());
    }
    std::cout << "CKexPkts: \n";
    ...
    if (hasNetworkConn) {
      ...
      for (auto &&recLayerMsg : recLayerMsgs)
        allKexPackets.insert(allKexPackets.end(), recLayerMsg.begin(),
                             recLayerMsg.end());
      ...
      auto reqPacket = TLS_composeRecordLayer(*tlss.TLS_writeAppData(
          Debugging::forceU8ViseArr(std::string_view{request})));
      pRecLayer(reqPacket);
      auto bytes = send(socket_, reqPacket.begin()->data(),
                        reqPacket.begin()->size(), 0);
      if (bytes <= 0) {
        std::cerr << "Failed to send request\n";
        perror("send");
        return EXIT_FAILURE;
    ...
    {
      Debugging::pu8Vec(Debugging::forceU8Vise(to_big_endian(tlss.m_state_)), 8,
                        true, "tls session state");
      ...
      Debugging::pu8Vec(
          Debugging::forceU8Vise(to_big_endian(*tlss.m_selectedCipherSuite)), 8,
          true, "selected cipher suite");
      Debugging::pu8Vec(
          Debugging::forceU8Vise(*tlss.m_selectedCompressionMethod), 8, true,
  ...
  unique_ptr_with_deleter<void> u{nullptr,
                                  [&](void *) { SSL_shutdown(ssl.get()); }};
  std::cout << "ssl conn\n";

  unique_ptr_with_deleter<X509> cert{SSL_get_peer_certificate(ssl.get()),
                                     X509_free};
  if (cert) {
    char *line =
