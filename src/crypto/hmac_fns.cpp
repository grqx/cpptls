...
std::vector<uint8_t> hmac_sha1(const HMAC_hashFnArgsType& args) {
    ...
    if (!mac)
        throw std::runtime_error("Failed to fetch HMAC EVP_MAC");
    ...
    char hashName[] = "SHA1";
    OSSL_PARAM params[] = {
        OSSL_PARAM_construct_utf8_string("digest", hashName, 0),
...
std::vector<uint8_t> hmac_sha256(HMAC_hashFnArgsType args) {
    ...
    if (HMAC_Init_ex(hmacCtx.get(), args.secret.data(), static_cast<int>(args.secret.size()), EVP_sha256(), nullptr) != 1 ||
