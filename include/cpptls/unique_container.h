...
class UniqueContainer final : public Container<T>
{
   public:
    ...
    template <typename... Args,
              typename = std::enable_if_t<(std::is_same_v<T, std::decay_t<Args>> && ...)>>
    ...
    constexpr UniqueContainer(ITER_T &&first, ITER_T &&last)
        : Container<T>(std::make_move_iterator(std::forward<decltype(first)>(first)),
    ...
    template <typename STORAGE_T,
              typename = std::enable_if_t<!std::is_convertible_v<std::decay_t<STORAGE_T>, T>>>
    constexpr UniqueContainer(STORAGE_T &&arg)
        : Container<T>(std::make_move_iterator(std::begin(arg)),
                       std::make_move_iterator(std::end(arg)))
    {
#ifndef NDEBUG
