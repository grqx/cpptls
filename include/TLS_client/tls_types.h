   ...
   public:
    ...
    TLSPlaintext toPacket(TLS_Version vsn) const
    {
        ...
        if (m_cont.size() > UINT24_MAX)
            throw std::overflow_error("Handshake content length overflowed UINT24_MAX");
...
class TLS_Alert final : public std::exception
{
   ...
   public:
    ...
    TLSPlaintext toPacket(TLS_Version vsn)
    {
        ...
        ret.recordVersion = vsn;
        ret.realCont = {
            static_cast<uint8_t>(m_aCode),
            static_cast<uint8_t>(m_fatal ? TLS_AlertLevel::Fatal : TLS_AlertLevel::Warning)};
        return ret;
    }
