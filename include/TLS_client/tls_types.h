...
typedef std::function<std::vector<uint8_t>(HMAC_hashFnArgsType)>
    HMAC_hashFnType;
...
// TODO
inline bool validateHandshakeType(HandshakeType ht) { return true; }
...
public:
  ...
  TLSPlaintext toPacket(TLS_Version vsn) const {
    ...
    if (m_cont.size() > UINT24_MAX)
      throw std::overflow_error(
          "Handshake content length overflowed UINT24_MAX");
    stdcopy_to_big_endian(static_cast<uint32_t>(m_cont.size()),
                          std::back_inserter(ret.realCont), 3);
    std::copy(m_cont.begin(), m_cont.end(), std::back_inserter(ret.realCont));
    return ret;
  }

  static std::optional<TLS_Handshake> parse(TLSPlaintext tpt,
                                            bool lazy = false) {
    if (tpt.contTyp != ContentType::Handshake)
      return std::nullopt;
    auto tptSize = tpt.realCont.size();
    if (tptSize < MIN_HANDSHAKE_PACKET_LEN)
      return std::nullopt;
    TLS_Handshake hs;
    hs.m_ht = static_cast<HandshakeType>(tpt.realCont[0]);
    if (!validateHandshakeType(hs.m_ht))
      return std::nullopt;
    auto contLen = from_big_endian<uint32_t, 3>(tpt.realCont.data() + 1);
    if (tptSize < MIN_HANDSHAKE_PACKET_LEN + contLen)
      return std::nullopt;
...
enum class TLS_AlertLevel : uint8_t { Warning = 0x01, Fatal = 0x02 };
...
public:
  ...
  TLS_Alert(TLS_AlertCode aCode, TLS_AlertLevel lvl)
      : m_aCode(aCode), m_fatal(lvl == TLS_AlertLevel::Fatal) {}
  TLSPlaintext toPacket(TLS_Version vsn) {
    TLSPlaintext ret;
    ret.contTyp = ContentType::Alert;
    ret.recordVersion = vsn;
    ret.realCont = {static_cast<uint8_t>(m_aCode),
                    static_cast<uint8_t>(m_fatal ? TLS_AlertLevel::Fatal
                                                 : TLS_AlertLevel::Warning)};
    return ret;
  }
  const char *what() const noexcept {
    if (desc)
      return desc->c_str();
    desc.emplace(strAlertDesc(static_cast<uint8_t>(m_aCode))
                     .value_or("<Unknown TLS Alert>"));
    return desc->c_str();
  }
};
