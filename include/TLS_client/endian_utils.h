...
template <template <typename> typename WRAPPER_T, typename T>
struct try_wrap<WRAPPER_T, T, std::void_t<WRAPPER_T<T>>>
    : public identity_type<WRAPPER_T<T>> {};
...
struct nth_type
    : public identity_type<std::tuple_element_t<s, std::tuple<Ts...>>> {};
template <size_t s, typename... Ts>
using nth_type_type = typename nth_type<s, Ts...>::type;

template <template <typename...> typename WRAPPER_T, typename void_,
          typename... Ts>
...
using try_get_underlying_unsigned =
    try_wrap_type<std::make_unsigned_t,
                  try_wrap_type<std::underlying_type_t, T>>;
...
template <typename SRC_T, typename DEST_T,
          typename = std::enable_if_t<
              is_valid_endian_conversion_subject_v<std::decay_t<SRC_T>>>>
void copy_to_ptr_big_endian(SRC_T &&src, DEST_T *dest,
                            size_t bytes = sizeof(std::decay_t<SRC_T>)) {
  if (bytes > sizeof(std::decay_t<SRC_T>))
    bytes = sizeof(std::decay_t<SRC_T>);
  auto hostVal =
      static_cast<try_get_underlying_unsigned<std::decay_t<SRC_T>>>(src);
...
auto to_big_endian(SRC_T &&src, size_t bytes = sizeof(std::decay_t<SRC_T>)) {
  ...
  RET_T ret = 0;
  copy_to_ptr_big_endian<decltype(src)>(std::forward<decltype(src)>(src), &ret,
                                        bytes);
  return ret;
}

template <typename SRC_T, typename DEST_T>
void stdcopy_to_big_endian(SRC_T &&src, DEST_T &&dest,
                           size_t bytes = sizeof(std::decay_t<SRC_T>)) {
...
template <typename RET_T_, size_t BYTES = sizeof(RET_T_), typename SRC_T,
          typename = std::enable_if_t<
              sizeof(RET_T_) / 2 < BYTES && BYTES <= sizeof(RET_T_) &&
