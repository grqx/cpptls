...
template <typename T>
using try_get_underlying_unsigned = try_wrap_type<std::make_unsigned_t, try_wrap_type<std::underlying_type_t, T>>;

template <typename T>
struct is_valid_endian_conversion_subject : public std::integral_constant<bool, std::is_integral_v<T> || std::is_enum_v<T>> {};

template <typename T>
inline constexpr bool is_valid_endian_conversion_subject_v = is_valid_endian_conversion_subject<T>::value;
...
template <
    typename SRC_T, typename DEST_T,
...
template <
    typename RET_T_, size_t BYTES = sizeof(RET_T_),
    typename SRC_T,
