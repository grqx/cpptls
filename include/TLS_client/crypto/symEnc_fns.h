...
std::vector<uint8_t> encryptAES_128_CBC(symEncFnArgsType args) {
  ...
  if (args.data.size() % 16 != 0 || args.data.size() == 0) {
    throw std::invalid_argument("Data size must be non-zero and a multiple of "
  ...
  // Finalize encryption
  if (EVP_EncryptFinal_ex(ctx.get(), ciphertext.data() + outlen, &outlen) !=
      1) {
...
CipherInfo getCipherInfo(const CipherSuite &cs) {
  if (cs == CipherSuite::TLS_RSA_WITH_AES_128_CBC_SHA) // AES_128_CBC
    return {ChatGPT4o::encryptAES_128_CBC, ChatGPT4o::decryptAES_128_CBC, 16,
            16, 16};
  UNREACHABLE;
}
