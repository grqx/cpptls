...
std::vector<uint8_t> encryptAES_128_CBC(symEncFnArgsType args) {
    ...
    if (args.data.size() % 16 != 0 || args.data.size() == 0) {
        throw std::invalid_argument("Data size must be non-zero and a multiple of 16 bytes for AES-128-CBC");
    }
    ...
    if (EVP_EncryptInit_ex(ctx.get(), EVP_aes_128_cbc(), nullptr, args.key.data(), args.iv.data()) != 1) {
    ...
    if (EVP_EncryptUpdate(ctx.get(), ciphertext.data(), &outlen, args.data.data(), args.data.size()) != 1) {
...
std::vector<uint8_t> decryptAES_128_CBC(symDecFnArgsType args) {
    ...
    if (EVP_DecryptInit_ex(ctx.get(), EVP_aes_128_cbc(), nullptr, args.key.data(), args.iv.data()) != 1) {
    ...
    // Prepare output buffer
    int outlen = 0;
    std::vector<uint8_t> plaintext(args.encryptedData.size() + EVP_CIPHER_block_size(EVP_aes_128_cbc()));

    // Perform the decryption
    if (EVP_DecryptUpdate(ctx.get(), plaintext.data(), &outlen, args.encryptedData.data(), args.encryptedData.size()) != 1) {
