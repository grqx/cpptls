...
std::vector<uint8_t> encRSA(const std::vector<uint8_t> &cont,
                            const EVP_PKEY_Ptr &pubKey) {
  ...
  if (EVP_PKEY_encrypt(ctx, encryptedData.data(), &outLen, cont.data(),
                       cont.size()) <= 0) {
    EVP_PKEY_CTX_free(ctx);
...
std::vector<uint8_t> serializePublicKey(const EVP_PKEY_Ptr &pubKey) {
  ...
  if (len <= 0) {
    throw std::runtime_error(
        "Failed to determine public key length: " +
...
std::vector<X509Ptr> parseCertificates(const std::vector<uint8_t> &certChain) {
  ...
  while (ptr + 3 <= end) { // Ensure there's enough data for the length field
    ...
    if (ptr + certLen > end) { // Check for data overflow
      throw std::runtime_error("Malformed certificate chain: certificate "
...
getPubKey(const std::vector<uint8_t> &certChain) {
  EVP_PKEY_Ptr serverPubKey;
  try {
    // Parse certificates (assumes certChain contains concatenated DER
    // certificates)
    ...
    for (const auto &cert : certificates) {
      ...
      char *issuer =
          X509_NAME_oneline(X509_get_issuer_name(cert.get()), nullptr, 0);
      if (subj && issuer) {
