...
private:
    ...
    const std::vector<uint8_t>& getKeyBlock() const noexcept
    {
        ...
        if (m_keyBlock.empty() && !m_masterSecret.empty()) {
            ...
            m_keyBlock = TLS_PRF(m_masterSecret, "key expansion"s, concatd, 2 * (getCSMACInfo().macKeyLength + getEncInfo().keyMaterial + getEncInfo().IVSize), getCSMACInfo().algo);
    ...
    const std::vector<uint8_t>& getServerWriteKey() const noexcept
    {
        if (m_serverWriteKey.empty() && !m_masterSecret.empty()) {
            const auto& kbb = getKeyBlock().begin();
            m_serverWriteKey = {kbb + 2 * getCSMACInfo().macKeyLength + getEncInfo().keyMaterial,
    ...
    const std::vector<uint8_t>& getClientWriteIV() const noexcept
    {
        if (m_clientWriteIV.empty() && !m_masterSecret.empty()) {
            ...
            m_clientWriteIV = {kbb + 2 * getCSMACInfo().macKeyLength + 2 * getEncInfo().keyMaterial,
                kbb + 2 * getCSMACInfo().macKeyLength + 2 * getEncInfo().keyMaterial + getEncInfo().IVSize};
        }
    ...
    const std::vector<uint8_t>& getServerWriteIV() const noexcept
    {
        if (m_serverWriteIV.empty() && !m_masterSecret.empty()) {
            const auto& kbb = getKeyBlock().begin();
            m_serverWriteIV = {kbb + 2 * getCSMACInfo().macKeyLength + 2 * getEncInfo().keyMaterial + getEncInfo().IVSize,
                kbb + 2 * getCSMACInfo().macKeyLength + 2 * getEncInfo().keyMaterial + 2 * getEncInfo().IVSize};
        }
...
public:
    ...
    TLS_Session(const TLS_Version& version, const std::list<CipherSuite>& css, const std::list<CompressionMethod>& cms)
    : m_vsn(version)
    {
    ...
    {
        // if (m_state_ != TLS_State_::Created) ;  // do something, maybe make ret optional
        ...
        if (sidSize > UINT8_MAX) throw std::overflow_error("TLS Session ID length overflowed UINT8_MAX");
        vec[38] = static_cast<uint8_t>(sidSize);
        if (sidSize)
            std::copy(m_sessionID.begin(), m_sessionID.end(), std::back_inserter(vec));

        // CipherSuites
        auto cipherSuitesSize = m_cipherSuites.size();
        if (cipherSuitesSize > UINT16_MAX) throw std::overflow_error("Cipher Suites length overflowed UINT16_MAX");
        ...
        if (compressionMethodsSize > UINT8_MAX) throw std::overflow_error("Compression Methods length overflowed UINT8_MAX");
        vec.push_back(static_cast<uint8_t>(compressionMethodsSize));
        if (compressionMethodsSize)
            std::copy(m_compressionMethods.begin(), m_compressionMethods.end(), std::back_inserter(vec));

        // Extensions, TODO
        // do not need to write extension length when there are no extensions
        // stdcopy_to_big_endian(static_cast<uint16_t>(0), std::back_inserter(vec), 2);

        // vec[1, 2, 3]: length of the rest of the packet
        size_t pktSize = vec.size() - 4;
        if (pktSize > UINT24_MAX) throw std::overflow_error("Client Hello Handshake length overflowed UINT24_MAX");
    ...
    /// @param serverHello const lvalue reference to a TLSPlaintext struct, should contain the server hello/server hello done packet
    /// @return true if succeeded, false if failed
    bool TLS_parseServerHello(const TLSPlaintext& serverHello)
    {
        if (m_state_ != TLS_State_::ClientHelloDone) return false;
        if (serverHello.contTyp != ContentType::Handshake) throw TLS_Alert(TLS_AlertCode::UnexpectedMessage, true);
        ...
        if (hs->m_ht == HandshakeType::ServerDn) {
            if (!m_serverRandom.size() || !m_serverPubKey.size() || !m_sessionID.size())
                throw TLS_Alert(TLS_AlertCode::HandshakeFailure, true);  // early server hello done
            ...
            if (m_serverRandom.size()) throw TLS_Alert(TLS_AlertCode::UnexpectedMessage, true);  // duplicate server hello

            auto serverVsn = from_big_endian<TLS_Version>(hs->m_cont.data() + parsed);
            if (serverVsn != m_vsn) throw TLS_Alert(TLS_AlertCode::ProtocolVersion, true);  // wrong proto ver
            parsed += sizeof(TLS_Version);

            std::copy(hs->m_cont.begin() + parsed, hs->m_cont.begin() + parsed + 32, std::back_inserter(m_serverRandom));
            parsed += 32;
            ...
            if (parsed + sizeof(CipherSuite) > csize) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // unexpected end-of-packet
            m_selectedCipherSuite = from_big_endian<CipherSuite>(hs->m_cont.data() + parsed);
            parsed += sizeof(CipherSuite);

            if (parsed + sizeof(CompressionMethod) > csize) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // unexpected end-of-packet
            ...
            if (parsed + sizeof(uint16_t) <= csize) {
                ...
                if (parsed + extLen > csize) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // unexpected end-of-packet
                parsed += extLen;
                throw TLS_Alert(TLS_AlertCode::UnsupportedExtension, true);
            }
        } else if (hs->m_ht == HandshakeType::Certificate) {
            if (csize < 3) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // unexpected end-of-packet
            auto certSize = from_big_endian<uint32_t, 3>(hs->m_cont.data());
            parsed += 3;

            if (parsed + certSize > csize) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // unexpected end-of-packet
            ...
            return false;
        if (parsed < csize) throw TLS_Alert(TLS_AlertCode::DecodeError, true);  // invalid redundant content
        std::copy(serverHello.realCont.begin(), serverHello.realCont.end(), std::back_inserter(m_handshakeMessages));
        return true;
    }
    ...
    std::optional<std::list<TLSPlaintext>> TLS_writeClientKex()
    {
        ...
        auto pmsEnc = ChatGPT4o::encRSA(m_preMasterSecret, ChatGPT4o::deserializePublicKey(m_serverPubKey));
        auto pmsEncSize = pmsEnc.size();
        if (!pmsEncSize) return std::nullopt;
        std::vector<uint8_t> hsCont;
        hsCont.reserve(2 + pmsEncSize);

        if (pmsEncSize > UINT16_MAX) throw std::overflow_error("Encrypted Pre-Master Secret length overflowed UINT16_MAX");
        ...
        if (!m_masterSecret.size()) {
            ...
            concatdRandoms.insert(concatdRandoms.end(), m_serverRandom.begin(), m_serverRandom.end());
            m_masterSecret = TLS_PRF(m_preMasterSecret, "master secret"s, concatdRandoms, 48, decideHMACAlgo(*m_selectedCipherSuite));
        }
        writeKeyLog();
    ...
    std::vector<uint8_t> encPlainText(const TLSPlaintext& tpt)
    {
        ...
        if (getEncInfo().blockSize > 0) {  // pad if using block cipher
            uint8_t padding_ = getEncInfo().blockSize - (dataWithHash.size() % getEncInfo().blockSize);
...
TLS_parseRecordLayerResult_ TLS_parseRecordLayer(const std::vector<uint8_t>& packet)
{
    ...
    while (parsed + 5 <= pSize) {
        ...
        parsed++;
        if (!validateContentType(cont.contTyp)) throw TLS_DecodeError("Invalid TLS Record Layer content type");
        ...
        std::copy(packet.data() + parsed, packet.data() + parsed + realContSize, std::back_inserter(cont.realCont));
        parsed += realContSize;
