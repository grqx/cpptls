...
public:
  ...
  TLS_DecodeError(Args &&...args)
      : TLS_BaseError(std::forward<decltype(args)...>(args...)) {}
...
public:
  TLS_AlertError(uint8_t code)
      : TLS_BaseError(strAlertDesc(code).value_or("<Unknown TLS Alert>")) {}

  static std::optional<TLS_AlertError> fromPacket(const TLSPlaintext &content,
                                                  bool returnWarning) {
    if (content.contTyp != ContentType::Alert)
      return std::nullopt;
    if (content.realCont.size() != 2)
      return std::nullopt;
    uint8_t lvl = content.realCont[0];
    bool isWarning = lvl == 1;
    if (!isWarning && lvl != 2)
      return std::nullopt;
    if (isWarning && !returnWarning)
      return std::nullopt;
  ...
  static void throwOnAlertPacket(const TLSPlaintext &content,
