...
inline void pu8Vec(const U8CONTAINER_ &u8vec, size_t alignTo = 8,
                   bool addLSep = true, const std::string &prefix = "",
...
void a() { hexToBytes(nullptr, nullptr, 0); }
...
std::vector<uint8_t> parseBytesArray(const std::string &str) {
  ...
  for (size_t i = 0; i < str.size(); i += 2) {
    ...
    if (d1 == ' ' || d1 == '\n' || d1 == '\r')
      continue;
    if (i + 1 >= str.size()) {
      std::cerr << "Redundant character '" << d1
    ...
    if (d2 == ' ' || d2 == '\n' || d2 == '\r') {
      i++;
      if (i + 1 >= str.size()) {
        std::cerr << "Redundant character '" << d2
                  << "' at the end of the string!\n";
        break;
      }
      d2 = str.at(i + 1);
