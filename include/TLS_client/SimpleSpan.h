...
template <typename T> class SimpSpan {
private:
  T *data_;
  size_t size_;
...
public:
  ...
  explicit SimpSpan(T *ptr_start, T *ptr_end) noexcept
      : data_(ptr_start), size_(ptr_end - ptr_start) {}

  template <size_t N> SimpSpan(T (&arr)[N]) noexcept : data_(arr), size_(N) {}
